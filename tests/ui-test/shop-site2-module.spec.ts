...
test("Add item to cart", { tag: ['@ui', '@smoke'] }, async ({ page, newSignupLoginPage }: any) => {
  ...
  await page.getByRole('link', { name: ' Products' ,timeout: 90000 }).click();
  if (await page.locator('iframe[name="aswift_3"]').isVisible()) {
    await page.locator('iframe[name="aswift_3"]').contentFrame().getByRole('button', { name: 'Close ad' }).click();
  }

  await page.getByRole('link', { name: ' View Product' ,timeout: 90000 }).nth(1).click();
  await page.getByRole('button', { name: ' Add to cart' ,timeout: 90000}).click();
